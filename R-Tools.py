...
def btn_search():
	...
	if info == 'Directory does not exists':
		b_search['state'] = 'normal'
		pop_up_error(n_error= 'Error', string='Složka neexistuje')
		return
...
################## Thunder button ###############################
def btn_thunder():
	ask = boxbox["question"](title='Are you sure about that?', message="Opravdu chcete pokračovat?")

	b_thunder['state'] = 'disabled'
	...
	if ask == 'yes':
		boxbox["info"](title="Process", message="Hotovo")
...
#################### helper function #################################
def pop_up_error(n_error, string):
	messagebox.showerror(title= n_error, message=string)
	canvas.create_text(
	244.0, 149.0,
	text = f"{string}",
	fill = "#000000",
